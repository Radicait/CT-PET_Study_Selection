...
This script:
  - Reads 'df_unique.csv' as the source DataFrame.
  ...
  - For each such row:
      ...
      * Writes a detailed report text file in the OUTPUT_DIR using the row_id as the filename.
  ...
  - Extraction tasks run concurrently using 5 threads. To safely write to the CSV file,
  ...
  - Resume logic is implemented: On re-run, rows that are already present (identified by their
    unique 'row_id') in the CSV are skipped.
"""
...
OUTPUT_DIR = '/home/sina/gradient-data/reports_data/'
from datetime import datetime
OUTPUT_CSV = f"extracted_report_{datetime.now().strftime('%Y-%m-%d')}.csv"
MAX_RETRIES = 3
...
def load_processed_row_ids() -> set:
    ...
    Returns:
        A set of row_id strings that have already been written to OUTPUT_CSV.
    """
    processed_ids = set()
    if os.path.exists(OUTPUT_CSV):
        try:
            ...
            if 'row_id' in df_existing.columns:
                processed_ids = set(df_existing['row_id'].dropna().tolist())
...
def write_report_text(row: pd.Series, extracted_data: Dict[str, Any]) -> None:
    ...
    and extracted data. The file is named using the row's 'row_id' value.
    ...
    try:
        with open(filename, "w", encoding="utf-8") as f:
            ...
            f.write(str(row['deid_english_report']))
            f.write("\n\n" + "=" * 50 + "\n\n")
...
def update_row_with_extraction(row: pd.Series, extracted_data: Dict[str, Any]) -> pd.Series:
    ...
    Expected keys in extracted_data:
        - Clinical_Reason, Primary_Diagnosis, Lung_Nodules,
          Lung_Hypermetabolic_Regions, Lymph_Nodes_Hypermetabolic_Regions,
          Other_Hypermetabolic_Regions, Tracer, Scan_Region,
          Blood_Glucose_Level, Waiting_Time, CT_Scan.
    
    This function maps these keys to the DataFrame columns:
        clinical_reason, primary_diagnosis, lung_nodules, lung_hypermetabolic,
        lymph_nodes_hypermetabolic, other_hypermetabolic, tracer, scan_region,
        blood_glucose_level, waiting_time, ct_scan.
    ...
    row['tracer'] = extracted_data.get("Tracer", "")
    row['scan_region'] = extracted_data.get("Scan_Region", "")
    row['blood_glucose_level'] = extracted_data.get("Blood_Glucose_Level", "")
    row['waiting_time'] = extracted_data.get("Waiting_Time", "")
    row['ct_scan'] = extracted_data.get("CT_Scan", "")
    row['success'] = True
    return row
...
def extract_with_retry(report_text: str, row_id: str) -> Dict[str, Any]:
    ...
    Args:
        report_text: The de-identified report text.
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            logging.info(f"Processing row_id {row_id}: Attempt {attempt}/{MAX_RETRIES}")
            extraction_result = extract_data(report_text)
            extracted_data = json.loads(extraction_result)
            logging.info(f"Extraction succeeded for row_id {row_id} on attempt {attempt}")
            return extracted_data
        except Exception as e:
            logging.error(f"Attempt {attempt} failed for row_id {row_id}: {e}")
            if attempt < MAX_RETRIES:
                sleep(RETRY_DELAY * attempt)
    logging.error(f"All extraction attempts failed for row_id {row_id}")
    return {}
...
def process_row(row: pd.Series, all_columns: list, processed_ids: set) -> None:
    ...
    Args:
        ...
        processed_ids: A shared set of already processed row_ids.
    """
    row_id = str(row.get('row_id', '')).strip()
    if not row_id:
        logging.error("Row missing row_id. Skipping.")
        return

    # Check if this row_id has been processed (using the shared lock)
    with csv_lock:
        if row_id in processed_ids:
            logging.info(f"Row_id {row_id} already processed, skipping.")
            return
    ...
    if pd.notna(clinical_reason) and str(clinical_reason).strip() != "":
        ...
        logging.info(f"Row_id {row_id} already had clinical_reason; written to CSV.")
        return

    # Otherwise, run extraction.
    report_text = row.get('deid_english_report', '')
    if not report_text:
        logging.error(f"Row_id {row_id} missing report text, skipping extraction.")
        row['success'] = False
        with csv_lock:
            with open(OUTPUT_CSV, 'a', encoding='utf-8', newline='') as csvfile:
                writer = csv.DictWriter(csvfile, fieldnames=all_columns)
    ...
    with csv_lock:
        with open(OUTPUT_CSV, 'a', encoding='utf-8', newline='') as csvfile:
            ...
            writer.writerow(row.to_dict())
        processed_ids.add(row_id)
    logging.info(f"Finished processing row_id {row_id} with success={row.get('success')}")
...
def main():
    ...
    Main function that orchestrates the extraction process:
      1. Loads the source CSV (df_unique.csv).
      2. Initializes the output directory and CSV.
      3. Loads already processed row_ids from the output CSV.
    ...
    # Determine the CSV header: use df columns and add 'success' if missing.
    all_columns = list(df.columns)
    if 'success' not in all_columns:
        all_columns.append('success')
    initialize_output_csv(all_columns)

    # Load the set of already processed row_ids.
